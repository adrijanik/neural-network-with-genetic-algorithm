...
def draw():
    ...
    plotly.offline.plot({
        ...
        "layout": Layout(title="Score function",xaxis=dict(title='generation'),yaxis=dict(title="max distance"))
    }, filename ='score_function.html', auto_open=False,
    )
...
#--------------------------------------------------------------        
    ...
    if timer.localPosition.y < 1.0:
        print("Create model of quadruped.")
...
#-------------------Create body---------------------------------------------------------------------------#
#---Something is wrong with this definition of body
#
